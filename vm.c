...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for(; a < newsz; a += PGSIZE){
    mem = kalloc(); //kalloc returns 1 page's VA
    if(mem == 0){
...
int deallocuvm2(pde_t *pgdir, uint oldsz, uint newsz, struct proc* p)
{
  ...
  for(; a  < oldsz; a += PGSIZE){
    ...
    if(!pte)
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
    else if((*pte & PTE_P) != 0){ //Present in memory
      pa = PTE_ADDR(*pte);
      if(pa == 0)
        panic("kfree");
      char *v = P2V(pa);
...
void freevm2(pde_t *pgdir, struct proc* p)
{
  uint i;
...
copyuvm_cow(pde_t *pgdir, uint sz, struct proc* p)
{
  ...
  for(i = 0; i < sz; i = i + PGSIZE)
  {
    ...
    *pte = (~ PTE_W) & *pte;  // read only parent page
    ...
    increment_rmap(pa, myproc()->pid);

  }
  lcr3(V2P(pgdir));   // flushteh TLB
  return d;
}
